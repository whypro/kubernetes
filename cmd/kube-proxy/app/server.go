...
func AddFlags(options *Options, fs *pflag.FlagSet) {
	...
	fs.StringVar(&options.config.MetricsBindAddress, "metrics-bind-address", options.config.MetricsBindAddress, "The IP address and port for the metrics server to serve on (set to `0.0.0.0` for all IPv4 interfaces and `::` for all IPv6 interfaces)")

...
func TestStaticPolicyName(t *testing.T) {
	policy, _ := NewStaticPolicy(topoSingleSocketHT, 1, topologymanager.NewFakeManager())
...
func TestStaticPolicyStart(t *testing.T) {
	testCases := []staticPolicyTest{
		...
		},
		{
			...
			expErr:          fmt.Errorf("container id: 0 cpuset: \"0-2\" overlaps with default cpuset \"2-11\""),
		},
		{
	...
	for _, testCase := range testCases {
		t.Run(testCase.description, func(t *testing.T) {
			p, _ := NewStaticPolicy(testCase.topo, testCase.numReservedCPUs, topologymanager.NewFakeManager())
...
func TestStaticPolicyAdd(t *testing.T) {
	...
	for _, testCase := range testCases {
		policy, _ := NewStaticPolicy(testCase.topo, testCase.numReservedCPUs, topologymanager.NewFakeManager())
...
func TestTopologyAwareAllocateCPUs(t *testing.T) {
	...
	for _, tc := range testCases {
		p, _ := NewStaticPolicy(tc.topo, 0, topologymanager.NewFakeManager())
		policy := p.(*staticPolicy)
		st := &mockState{
